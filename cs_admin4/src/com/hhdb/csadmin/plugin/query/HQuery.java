...
//import com.hhdb.csadmin.common.ui.textEdit.QueryEditorUi;
...
import com.hhdb.csadmin.plugin.query.dataselect.HHTablePanel;
...
public class HQuery {
	...
	private void init() {
		...
		openBtn = new JButton("加载", new ImageIcon(
				ClassLoader.getSystemResource("icon/open.png")));
		saveBtn = new JButton("保存", new ImageIcon(
				ClassLoader.getSystemResource("icon/save.png")));
		saveasBtn = new JButton("另保存", new ImageIcon(
		...
		saveBtn.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				String filename = JOptionPane.showInputDialog(null, "输入查询名",
						"查询名", JOptionPane.PLAIN_MESSAGE);
				if (filename == null) {
					return;
				}
				if (filename.trim().isEmpty()) {
					JOptionPane.showMessageDialog(null, "查询名不能为空", "提示",
				...
				try {
					File file = new File(CommonsHelper.getClassPath()
							...
							+ (String) schemabox.getSelectedItem() + "/");
					if (!file.exists()) { //
						file.mkdirs();
					}
					for (File fl : file.listFiles()) {
						if (fl.getName()
								.substring(0, fl.getName().length() - 4)
								.equals(filename)) {
							JOptionPane.showMessageDialog(null, "查询名已存在", "提示",
									JOptionPane.WARNING_MESSAGE, null);
							return;
						}
					}

					File files = FileUtils.getFile(file.getPath() + "/"
							+ filename + ".sql");
					FileUtils.write(files, queryUi.getTextArea().getText(),
							"utf-8");

					CmdEvent cmdEvent = new CmdEvent(queryPlugin.PLUGIN_ID,
							"com.hhdb.csadmin.plugin.tree",
							"RefreshAddTreeNodeEvent");
					cmdEvent.addProp("schemaName",
							(String) schemabox.getSelectedItem());
					cmdEvent.addProp("treenode_type", "query");
					queryPlugin.sendEvent(cmdEvent);

				} catch (IOException e1) {
					JOptionPane.showMessageDialog(null, "查询名有误或已存在", "提示",
							JOptionPane.WARNING_MESSAGE, null);
					e1.printStackTrace();
				} finally {
					try {
...
//					}
	...
	private void showQueryData(int position, Query query) {
		...
		try {
			...
			//新建一个表格
		    hhTbPanel = new HHTablePanel(true,queryPlugin);
			...
			for(Object type : list.get(1)){
				typename.add(type.toString());
			}
			...
			namelist.add("");
			namelist.addAll(colname);
			hhTbPanel.setDataList(namelist, typelist,list);
...
//			JScrollPane scroll = new JScrollPane(hhTbPanel);
//			scroll.getViewport().setBackground(Color.WHITE);
//			scroll.setBorder(null);
			outPanel.add(hhTbPanel, BorderLayout.CENTER);
