...
public class SqlOperationService {
	...
	public Connection getConn() {		
		try {
			...
			if(null == connection || connection.isClosed() || !ConnUtil.isConnected(connection)){   
				...
				CmdEvent obtainRowsEvent = new CmdEvent(hbook.PLUGIN_ID, toId,"GetConn");
	...
	public void deleteFolder(BaseTreeNode treeNode) {
		try {
			...
			if( (null == listdir || listdir.size() == 0) && (null == lisrobj || lisrobj.size() == 0) && (null == list || list.size() == 0) ){    //判断里面没有
				...
				JOptionPane.showMessageDialog(null, "请先删除子级元素", "消息",JOptionPane.ERROR_MESSAGE);
			}
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, e.getMessage(), "消息",JOptionPane.ERROR_MESSAGE);
		}
