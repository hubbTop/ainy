...
//					String foreName = baseTable.getValueAt(baseTable.getSelectedRow(), 0).toString();
					dels.add(baseTable.getValueAt(baseTable.getSelectedRow(), 0).toString());
					// CommonDAO.dropForeign(nodes,tableName, foreName);
				}
	...
	public void editForeign() throws Exception {
		...
		for (int i = 0; i < li.size(); i++) {
			...
			String columns = "";
			for (String str : m.get("indkey").toString().split(" ")) {
				columns += tabp.sqls.getIndexColumn(Integer.parseInt(m.get("conindid").toString()),Integer.parseInt(str)) + ",";
			}
			if (columns.length() > 0) {
				columns = columns.substring(0, columns.length() - 1);
			}
	...
	public String editForeignSql() {
		...
		if (dels.size() > 0) {
			for (String str : dels) {
				sqlBuffer.append(ALTER+" TABLE " ).append("\""+tabp.getSchemaName()+"\".\""+tabp.getTableName()+"\"").append(" DROP CONSTRAINT ").append(str.trim());
