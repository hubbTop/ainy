...
public class HandleIndexPanel extends JPanel implements TableModelListener, CreateTableSQLSyntax {
	...
	public void editIndex() throws Exception {
		...
		for (int i = 0; i < li.size(); i++) {
			...
			if (m.get("唯一键").toString().equals("t") || m.get("唯一键").toString().equals("true")) {
				m.put("唯一键", true);
			} else {
				m.put("唯一键", false);
			}
			indexbean.setUnique(Boolean.parseBoolean(m.get("唯一键").toString()));
			String columns = "";
		...
		for (int i = 0; i < rows; i++) {
			if (indexTable.getValueAt(i, 5) != null) {   //修改
				...
				if (!coln.getDefinition().equals(CommonsHelper.nullOfStr(indexTable.getValueAt(i, 6))) && lsdef.length() == 0) {
					comments.append(NEW_LINE).append(" COMMENT ON INDEX ").append("\""+tabp.getSchemaName()+"\"").append(DOT)
							.append(indexTable.getValueAt(i, 0)).append(" IS ")
							.append("'" + indexTable.getValueAt(i, 6) + "'").append(SEMI_COLON);
				} else {
