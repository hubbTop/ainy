...
public class HandleRulePanel extends JPanel implements TableModelListener,CreateTableSQLSyntax {
	...
	public HandleRulePanel(TableEditPanel tableeditpanel)throws Exception {
		...
		tablePanel = new BaseTablePanel(new int[] { 1 });
	...
	public String editRuleSql() {
		...
		if (dels.size() > 0) {       
			for (String st : dels) {
				sqlBuffer.append(DROP).append(RULE).append(st).append(ON)
						.append("\"" + tabp.getSchemaName() + "\"").append(DOT)
						.append("\"" + tabp.getTableName() + "\"").append(SEMI_COLON)
						.append(NEW_LINE);
			}
		}
