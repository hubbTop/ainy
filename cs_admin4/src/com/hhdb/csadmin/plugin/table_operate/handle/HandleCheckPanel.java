...
public class HandleCheckPanel extends JPanel implements TableModelListener, CreateTableSQLSyntax {
	...
	public String editCheckSql() {
		...
		if (dels.size() > 0) {
			for (String st : dels) {
				sqlBuffer.append(ALTER_TABLE).append("\""+tabp.getSchemaName()+"\"").append(DOT).append("\""+tabp.getTableName()+"\"").append(NEW_LINE);
				sqlBuffer.append(DROP_CONSTRAINT).append(st).append(SEMI_COLON).append(NEW_LINE);
			}
		}
		for (int i = 0; i < rows; i++) {
			...
			if (baseTable.getValueAt(i, 2) != null) {   //修改
				...
				if (!bean.getUnqiueName().equals(baseTable.getValueAt(i, 0).toString()) || !bean.getColumns().equals(baseTable.getValueAt(i, 1).toString())) {
					cont.append(NEW_LINE).append(ALTER_TABLE).append("\""+tabp.getSchemaName()+"\"").append(DOT).append("\""+tabp.getTableName()+"\"").append(NEW_LINE);
					cont.append(DROP_CONSTRAINT).append(bean.getUnqiueName()).append(COMMA).append(NEW_LINE);
