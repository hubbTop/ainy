...
//				sqlBuffer.append("alter table "+tabp.getSchemaName()+"\".\""+tabp.getTableName()+" drop constraint "+tabp.getTableName() + "_pkey"+";");
//				sqlBuffer.append("alter table "+tabp.getSchemaName()+"\".\""+tabp.getTableName()+" add CONSTRAINT sss PRIMARY KEY (id);");
//			}
		}
	}
	...
	public String editTableSql() {
		...
		if(tabp.getSign()){
			...
			sqlBuffer.append(coldefault.toString());	
		}else{
			if (iskey) {  //组装主键
				if (parkey != "(") {
					sqlBuffer.append(NEW_LINE).append(ALTER_TABLE).append(SPACE).append("\""+tabp.getSchemaName()+"\".\""+tabp.getTableName()+"\"")
					.append(ADD).append(PRIMARY).append(KEY).append(parkey.substring(0, parkey.length() - 1) + ");");
				}
			}
		}
