...
import com.hhdb.csadmin.common.csv.writer.CsvWriter;
...
public class TreeService {
	...
	public void executeDQL(String sql) throws Exception{
		String toID = "com.hhdb.csadmin.plugin.conn";
		CmdEvent event = new CmdEvent(htree.PLUGIN_ID, toID, "ExecuteQueryDQLBySqlEvent");
		event.addProp("sql_str", sql);
		HHEvent ev = htree.sendEvent(event);
		if (ev instanceof ErrorEvent) {
			throw new Exception(((ErrorEvent) ev).getErrorMessage());
	...
	public List<List<String>> getListStrBySql(String sql) throws Exception {
		String toID = "com.hhdb.csadmin.plugin.conn";
		CmdEvent event = new CmdEvent(htree.PLUGIN_ID, toID, "ExecuteCSVBySqlEvent");
		event.addProp("sql_str", sql);
		HHEvent ev = htree.sendEvent(event);
		if (ev instanceof ErrorEvent) {
			throw new Exception(((ErrorEvent) ev).getErrorMessage());
	 ...
	 * @throws Exception
	 */
	public List<Map<String, Object>> getListMapBySql(String sql) throws Exception {
		String toID = "com.hhdb.csadmin.plugin.conn";
		CmdEvent event = new CmdEvent(htree.PLUGIN_ID, toID, "ExecuteListMapBySqlEvent");
		event.addProp("sql_str", sql);
		HHEvent ev = htree.sendEvent(event);
		if (ev instanceof ErrorEvent) {
			throw new Exception(((ErrorEvent) ev).getErrorMessage());
		...
		List<Map<String, Object>> list = (List<Map<String, Object>>)ev.getObj();
		return list;
	}
	 ...
	 * @throws Exception
	 */
	public String pdTableName(String tableName,int schemaId,int flag) throws Exception{
		String tablename;
		if(flag==0){
			tablename = tableName;
		}else{
		...
		String s = "select count(*) from "+StartUtil.prefix+"_class where relname='"+tablename+"' and relnamespace="+schemaId;
	...
	public List<Map<String, Object>> getSchemaIdName() throws Exception {
		...
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.SCHEMA,
				"prop_coll");
		List<Map<String, Object>> rs = SqlQueryUtil.select(
				conn, sb.getSql());
	...
	public void sendAttr(String csvuEventStr, String ico) {
		CmdEvent event = new CmdEvent(htree.PLUGIN_ID, "com.hhdb.csadmin.plugin.attribute", "Show");
	...
	public String getcsvAttrCollection(List<Map<String, Object>> rs, SqlBean sb)
		...
		if (rs.size() > 0) {
			...
			for(Map<String, Object> m : rs){
				...
				for (int i = 0; i < sb.getColumnKeys().size(); i++) {		
					str2[i] = m.get(sb.getColumnKeys().get(i))!=null?m.get(sb.getColumnKeys().get(i)).toString():"";
				}
				data.add(str2);
			}
	...
	public String getAttrCollectionSchema() throws Exception{
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.SCHEMA, "prop_coll");
	...
	public String getAttrCollectionView(String schemaname) throws Exception{
		Object[] params = new Object[1];
		params[0] = "'"+schemaname+"'";
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.VIEW, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	...
	public  String getAttrCollectionFuction(String schemaname) throws Exception{
		Object[] params = new Object[1];
		params[0] = "'"+schemaname+"'";
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.FUNCTION, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	...
	public  String getAttrCollectionTypes(int schemaoid) throws Exception{
		...
		params[0] = schemaoid;
		params[1] = schemaoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.TYPE, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	...
	public  String getAttrCollectionSequence(String schemaname) throws Exception{
		Object[] params = new Object[1];
		params[0] = "'"+schemaname+"'";
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.SEQUENCE, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	...
	public String getAttrCollectionCol(int tableoid) throws Exception{
		Object[] params = new Object[1];
		params[0] = tableoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.COLUMNS, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	...
	public  String getAttrCollectionConstraints(int tableoid) throws Exception{
		Object[] params = new Object[1];
		params[0] = tableoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.CONSTRAINTS, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	/**
	...
	public String getAttrCollectionIndex(int tableoid) throws Exception{
		Object[] params = new Object[1];
		params[0] = tableoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.INDEXS, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	/**\
	...
	public String getAttrCollectionTrigger(int tableoid) throws Exception{
		Object[] params = new Object[1];
		params[0] = tableoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.TABLE_TRIGGERS, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	...
	public String getAttrCollectionRule(int tableoid) throws Exception{
		Object[] params = new Object[1];
		params[0] = tableoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.RULES, "prop_coll");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		return getcsvAttrCollection(rs, sb);
	}
	...
	public String getAttrType(int schemaoid,int typeoid) throws Exception {
		...
		params[2] = schemaoid;
		params[3] = typeoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.TYPE, "prop_item");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		Map<String,String> map = new HashMap<String,String>();
		Map<String,Object> m = rs.get(0);
		for (int i = 0; i < sb.getColumnKeys().size(); i++) {
			map.put(sb.getColumnNames().get(i), 
					m.get(sb.getColumnKeys().get(i))!=null?m.get(sb.getColumnKeys().get(i)).toString():"");
		}
	 ...
	 * @throws Exception
	 */
	public String getCreateSqlType(String schemaName,int schemaoid,int typeoid) throws Exception{
		Object[] params = new Object[42];
		params[0] = schemaoid;
		params[1] = typeoid;
		params[2] = schemaoid;
	...
	private String getCompositeCode(int typeoid, String name) throws Exception {
		Object[] params = new Object[1];
		params[0] = typeoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.TYPE, "compositesource");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
	...
	private String getbasicCode(int typeoid, String name) throws Exception {
		Object[] params = new Object[1];
		params[0] = typeoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.TYPE, "basicsource");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
	...
	private String getEnumCode(int typeoid, String name) throws Exception {
		Object[] params = new Object[1];
		params[0] = typeoid;
		SqlBean sb = HHSqlUtil.getSqlBean(HHSqlUtil.ITEM_TYPE.TYPE, "enumsource");
		List<Map<String, Object>> rs = getListMapBySql(sb.replaceParams(params));
		String result="-- DROP TYPE " + name + ";\n";
		result=result+"\n";
		String enums = "";
	 ...
	 * @throws Exception 
	 */
	public BaseTreeNode[] InitTreeNode(String xmlpath)
			throws Exception {
		...
		if (nodechidrens.getLength() > 0) {
			for (int i = 0; i < nodechidrens.getLength(); i++) {
				if (nodechidrens.item(i).getNodeName().equals("node")) {
					...
					treenodelist
							.add(recursionTreeNode(null, nodechidren, null,null));
				}
			}
		}
	 ...
	 * @throws Exception 
	 */
	public BaseTreeNode recursionTreeNode(BaseTreeNode parent,
			Element node, String rename,String id) throws Exception {
	...
	public void refreshDBCollection(BaseTreeNode treeNode) throws Exception{
		try {
			...
			// 加上这两句是可以让节点刷新后直接展开
			TreePath path = new TreePath(htree.getTree()
	 ...
	 * @throws Exception
	 */
	public void refreshTablespaceCollection(BaseTreeNode treeNode) throws Exception{
		try {
	 ...
	 */
	public void refreshSchema(BaseTreeNode treeNode)
			throws URISyntaxException, ParserConfigurationException, SAXException, IOException {
		Document doc = XmlFileUtil.getResXmlDoc("com/hhdb/csadmin/plugin/tree/xml/tree_schema.xml");
	...
	public void refreshTable(BaseTreeNode treeNode) throws Exception {
		Document doc = XmlFileUtil.getResXmlDoc("com/hhdb/csadmin/plugin/tree/xml/tree_table.xml");
		...
		try {
			List<Map<String, Object>> tableIdNames = getTableIdName(((BaseTreeNode) treeNode
					.getParentBaseTreeNode()).getMetaTreeNodeBean()
					.getName());
			treeNode.removeAllChildren();
	...
	public void refreshFunction(BaseTreeNode treeNode) throws Exception {
		try {
			...
			for(Map<String, Object> m:functionidnames) {
				...
				MetaTreeNodeBean tablemtn = new MetaTreeNodeBean();
				tablemtn.setName(m.get("proname").toString()+ "(" + m.get("arguments").toString() + ")");
				tablemtn.setId(Integer.parseInt(m.get("id").toString()));
	 ...
	 * @throws Exception
	 */
	public void refreshTreeNodeBySchemaName(String schemaName,String type) throws Exception{
		...
		e:for(BaseTreeNode dbNode:rootTreeNode.getChildNode()){
			if(dbNode.getType().trim().equals(TreeNodeUtil.DB_ITEM_TYPE)){
				for(BaseTreeNode schemasNode:dbNode.getChildNode()){
					if(schemasNode.getType().trim().equals(TreeNodeUtil.SCHEMA_TYPE)){
						for(BaseTreeNode schemaNode:schemasNode.getChildNode()){
							if(schemaNode.getMetaTreeNodeBean().getName().trim().equals(schemaName)){
		...
		if(tempSchemaNode!=null){
			if(type.trim().equalsIgnoreCase("table")){
				for(BaseTreeNode tableTreeNode : tempSchemaNode.getChildNode()){
					if(tableTreeNode.getType().equalsIgnoreCase(TreeNodeUtil.TABLE_TYPE)){
				...
				for(BaseTreeNode viewTreeNode : tempSchemaNode.getChildNode()){
					if(viewTreeNode.getType().equalsIgnoreCase(TreeNodeUtil.VIEW_TYPE)){
				...
				for(BaseTreeNode functionTreeNode : tempSchemaNode.getChildNode()){
					if(functionTreeNode.getType().equalsIgnoreCase(TreeNodeUtil.FUN_TYPE)){
				...
				for(BaseTreeNode sequenceTreeNode : tempSchemaNode.getChildNode()){
					if(sequenceTreeNode.getType().equalsIgnoreCase(TreeNodeUtil.SEQ_TYPE)){
				...
				for(BaseTreeNode typeTreeNode : tempSchemaNode.getChildNode()){
					if(typeTreeNode.getType().equalsIgnoreCase(TreeNodeUtil.TYPE_TYPE)){
				...
				for(BaseTreeNode queryTreeNode : tempSchemaNode.getChildNode()){
					if(queryTreeNode.getType().equalsIgnoreCase(TreeNodeUtil.SELECT_TYPE)){
	...
	public void refreshToolBar(String showBar){
		...
		for(String bname:toolButtonNames){
			CmdEvent toolbarShowEvent = new CmdEvent(htree.PLUGIN_ID, "com.hhdb.csadmin.plugin.tool_bar", "ToolbarShowEvent");
