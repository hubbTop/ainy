...
public final class CsvWriter {
     ...
     * @throws IOException 
     */
    public void writeSbf(final StringBuffer sbf,final Collection<String[]> data) throws IOException{
    	Objects.requireNonNull(data, "data must not be null");
        final CsvAppender appender = append(sbf);
        for (final String[] values : data) {
            appender.appendLineSbf(values);
        }
    }
    ...
    public CsvAppender append(final Writer writer) {
        return new CsvAppender(null,Objects.requireNonNull(writer, "writer must not be null"),
            fieldSeparator, textDelimiter, alwaysDelimitText, lineDelimiter);
    }
    
    public CsvAppender append(final StringBuffer sbf) {
        return new CsvAppender(Objects.requireNonNull(sbf, "sbf must not be null"),null,
            fieldSeparator, textDelimiter, alwaysDelimitText, lineDelimiter);
    }
